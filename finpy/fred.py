...
class Fred(object):
    ...
    def get1d(self, first="series", second="observations", **kwargs):
        if second != "":
            url = "http://api.stlouisfed.org/fred/%s/%s?api_key=%s" % (first, second, self.api_key)
        else:
            url = "http://api.stlouisfed.org/fred/%s?api_key=%s" % (first, self.api_key)
        if kwargs is not None:
            url += '&' + urlencode(kwargs)
        print(url)
        root = self.__fetch_data(url)
        if root is None:
            raise ValueError('No info exists for the query: ' +  url)
